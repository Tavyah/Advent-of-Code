...
def get_path_of_folder(folder: str) -> str:
    FILEPATH = os.path.dirname(os.path.realpath(__file__))
    FOLDER = folder
    return os.path.join(FILEPATH, FOLDER)
...
def get_folder_get_list_with_foldernames(filepath_to_folder_to_search: str) -> list:
    folders = [folder for folder in os.listdir(filepath_to_folder_to_search)]
    return folders
